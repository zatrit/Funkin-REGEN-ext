...
import Conductor.ClassConductor;
import flixel.FlxSprite;
import flixel.group.FlxSpriteGroup;

using StringTools;

class Cutscene1 extends FlxSpriteGroup implements Cutscene
{
	var bg:FlxSprite;
	...
	override function update(elapsed:Float)
	{
		...
		if (startUpdate)
		{
			if (thankYouSoMuchAPieceOfShit.animation.curAnim != null)
			{
				...
				if (thankYouSoMuchAPieceOfShit.animation.curAnim.name == "cutscene"
					&& thankYouSoMuchAPieceOfShit.animation.curAnim.finished)
				{
					visible = false;
					startUpdate = false;
					finishCallback();
